...
async function verifyPendingRoute() {
  try {
    console.log('Verifying pending events route with new endpoint...');
    ...
    // Step 2: Try to access the NEW pending events endpoint
    console.log('\nStep 2: Accessing NEW pending events endpoint...');
    try {
      response = await axios.get(`${API_URL}/api/events/admin/pending`, {
        headers: {
